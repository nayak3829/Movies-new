...
export function WhatsAppPopup() {
  ...
  useEffect(() => {
    const alreadySeen = sessionStorage.getItem('whatsapp_popup_seen');
  ...
  const handleClose = () => {
    sessionStorage.setItem('whatsapp_popup_seen', '1');
    setIsClosing(true);
  ...
  const handleJoin = () => {
    window.open('https://whatsapp.com/channel/0029Vadk2XHLSmbX3oEVmX37', '_blank', 'noopener,noreferrer');
    handleClose();
  };
  ...
  return (
    ...
    >
      <div
        className={`absolute inset-0 bg-gradient-to-br from-black/90 via-black/85 to-emerald-950/60 backdrop-blur-xl transition-opacity duration-500 ${isAnimating ? 'opacity-100' : 'opacity-0'}`}
      />
