...
export function WhatsAppPopup() {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative bg-[#0f1729] border border-gray-700/50 rounded-2xl p-8 max-w-sm w-full shadow-2xl animate-in fade-in zoom-in duration-300">
        {/* Close Button */}
        <button
          onClick={handleClose}
          className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full border border-gray-600 text-gray-400 hover:text-white hover:border-gray-400 transition-colors"
        ...
        <button
          onClick={handleJoin}
          className="w-full py-3 px-6 bg-[#25D366] hover:bg-[#20bd5a] text-white font-semibold rounded-full transition-colors duration-200"
        >
          Join Channel
        </button>
      </div>
